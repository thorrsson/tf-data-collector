...
if [[ -n "$SUBSCRIPTION_ID" ]]; then
    ...
    echo "Searching for subscriptions matching: $SUBSCRIPTION_NAME"
    ...
    ALL_SUBS=$(az account list $AZURE_CLI_WARNINGS --query "[].{name:name, id:id}" -o json)
    ...
    if [[ $MATCH_COUNT -eq 0 ]]; then
        echo "Error: No subscriptions found matching '$SUBSCRIPTION_NAME'"
        echo ""
    ...
    echo "Found subscription: $SUBSCRIPTION_NAME_FULL"
    echo "Subscription ID: $SUBSCRIPTION_ID"
else
    # Use current subscription
    echo "No subscription specified, using current Azure CLI context"
    SUBSCRIPTION_ID=$(az account show --query id -o tsv)
    SUBSCRIPTION_NAME_FULL=$(az account show --query name -o tsv)
    
    echo "Current subscription: $SUBSCRIPTION_NAME_FULL"
    echo "Subscription ID: $SUBSCRIPTION_ID"
fi
...
echo "Collecting data from subscription..."
